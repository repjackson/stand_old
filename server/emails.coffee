...
#
#
# Accounts.emailTemplates.siteName = 'stand';
# Accounts.emailTemplates.from = 'stand <noreply@thestand.app>';
# #
...
#    subject: -> "stand activation"
#    text: (user, url)-> "hi #{user.username}.\n please verify your e-mail by clicking here: #{url}.\n\n If you did not request this verification, please ignore this email. If you feel something is wrong, please contact support: admin@thestand.app."
#
#
...
#
# Mailer.config
#     from: 'stand <admin@thestand.app>',     # Default 'From:' address. Required.
#     replyTo: 'stand <admin@thestand.app>',  # Defaults to `from`.
...
#             subject: 'new stand enrollment'                     # Required.
#             template: 'admin_enrollment_email'               # Required.
#             replyTo: 'stand admin <admin@thestand.app>'      # Override global 'ReplyTo: ' option.
#             from: 'stand admin <admin@thestand.app>'         # Override global 'From: ' option.
...
#         # to: ['stand admin <admin@thestand.app>',"#{to_user.username} <#{to_user.emails[0].address}>"]          # 'To: ' address. Required.
#         Mailer.send
#             to: ['stand admin <eric@thestand.app>']          # 'To: ' address. Required.
#             subject: 'stand message'                     # Required.
...
#         if user.emails and user.emails[0] then console.log user.emails[0]
#         Mailer.send
#             to: ['stand admin <admin@thestand.app>',"#{user.username} <#{user.emails[0].address}>"]          # 'To: ' address. Required.
#             subject: 'the stand membership'                     # Required.
